$(function(){
		...
		.then(data => {
			data.forEach(element => {
				if(element.id == $cinemaId){
					$("#selectCinema").append(`<option value="${element.id}" selected>${element.name}</option>`)		
				}else{
		...
		success: function(data, status){
			if(status == "success"){
				...
				data.theaterList.forEach(element => {
					if(element.id == $theaterId){
						$('#selectTheater').append(`<option value="${element.id}" selected>${element.theaterNum}관</option>`)	
					}else{
	...
	$('#selectStatus').children(`[value='${$showStatus}']`).prop('selected', true);
	...
	$('#updateBtn').click(function(){
		...
		frm.attr('action', '/admin/show/update');
		let showDateTime = "";
...
function loadMovieSelect(data){
	data.forEach(element => {
		if(element.id == $movieId){
			$("#selectMovie").append(`<option value="${element.id}" selected>#${element.id} : ${element.title}</option>`);
		}else{
